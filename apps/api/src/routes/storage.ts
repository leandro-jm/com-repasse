...
import { MAX_FOTOS_POR_NEGOCIO, MAX_FOTOS_CAMPANHA } from '@crm/shared';
...
/**
 * Upload de fotos de uma campanha (tela de disparo manual). Mesmo bucket público
 * `fotos`, path {tenant}/campanhas/{campanhaId}/... — RLS de storage valida o tenant.
 */
storageRoutes.post('/campanhas/:id/fotos', async (c) => {
  const db = c.get('db');
  const tenantId = c.get('claims').tenant_id!;
  const campanhaId = c.req.param('id');

  // garante que a campanha pai pertence ao tenant ativo (RLS filtra o select)
  ...
  // limite de fotos por campanha
  if (base + files.length > MAX_FOTOS_CAMPANHA) {
    return c.json(
      {
        error: `Máximo de ${MAX_FOTOS_CAMPANHA} fotos por campanha. Já existem ${base}; você pode enviar mais ${Math.max(0, MAX_FOTOS_CAMPANHA - base)}.`,
      },
      422,
    );
  }
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const safe = file.name.replace(/[^\w.-]/g, '_');
    const path = `${tenantId}/campanhas/${campanhaId}/${Date.now()}-${i}-${safe}`;
    const buf = new Uint8Array(await file.arrayBuffer());
